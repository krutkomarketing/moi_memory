...
async function reconstructPage(rawText, currentBlocks, currentHeader = {}) {
  const headerState = [
    `- Имя: ${currentHeader.name ? `"${currentHeader.name}" (уже заполнено — НЕ трогай)` : 'НЕ ЗАПОЛНЕНО — если в рассказе есть имя, верни его в SET_HEADER'}`,
    `- Даты жизни: ${currentHeader.dates ? `"${currentHeader.dates}" (уже заполнены — НЕ трогай)` : 'НЕ ЗАПОЛНЕНЫ — если в рассказе есть годы рождения/смерти, верни их в SET_HEADER'}`,
    `- Город: ${currentHeader.city ? `"${currentHeader.city}" (уже заполнен — НЕ трогай)` : 'НЕ ЗАПОЛНЕН — если в рассказе упомянут город, верни его в SET_HEADER'}`,
  ].join('\n');
  ...
  const system = [
    ...
    '4. SET_HEADER: верни эту команду ПЕРВОЙ, если в рассказе есть имя покойного, годы жизни или город: { "action": "SET_HEADER", "name": "Имя Отчество Фамилия", "born": "1921", "died": "1986", "city": "Минск", "epitaph": "..." }. Поля, которых нет в рассказе, опусти. Поле epitaph заполняй ВСЕГДА: сочини короткую (1–2 предложения) тёплую уважительную эпитафию по сути рассказа.',
    '',
    'ВАЖНО — правила распределения:',
    '- Распредели ВЕСЬ рассказ по разделам: детство → childhood, учёба/институт/школа → education, работа/завод/профессия → career, война/армия/служба → military, увлечения/рыбалка/моделирование → hobbies, жена/дети/родители → family, пенсия/последние годы/чем запомнился → legacy.',
    '- Если на странице уже есть раздел с пустым текстом (см. текущие блоки), а в рассказе есть подходящие факты — верни UPDATE именно для его ключа, не создавай дубликат.',
    '- НИ ОДИН факт из рассказа не должен остаться без раздела.',
    '- Если после распределения какой-то из текущих блоков остался бы пустым (фактов для него нет) — верни для него DELETE, пустые разделы на странице не нужны.',
    '',
